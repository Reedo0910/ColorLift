...
由於拾彩目前未啟用程式簽署（因為作為興趣使然的個人專案，我並未支付 Apple 開發者帳號的年費），因此在 macOS 上首次運行時，會觸發系統的安全警告。

您只需要進行一些簡單的設定即可使用它，請參閱官方文件：[打開來自未知開發者的 Mac App](https://support.apple.com/guide/mac-help/mh40616/mac)。

### 系統設定中顯示拾彩已經獲得了「螢幕錄影與系統錄音」的權限，為什麼仍然提示需要獲取權限？（或為什麼無法正常取色？）

**該問題通常在安裝應用程式更新後出現，您需要重置拾彩的系統權限：**

(macOS 15.0 以上版本) 請到 系統設定 > 隱私與安全性 > 螢幕錄影與系統錄音 > 選取 ColorLift（拾彩） > 點擊列表左下角的減號鍵（`-`）將 ColorLift 從權限列表中移除（可能需要輸入您的系統密碼進行確認） > 重新啟動拾彩，並重新授予「螢幕錄影與系統錄音」權限。
